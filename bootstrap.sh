...
REPO_URL="${REPO_URL:-https://github.com/bacproxmox/homelabv2.4.3.git}"
INSTALL_DIR="${INSTALL_DIR:-/root/homelabv2.4.3}"
BRANCH="${BRANCH:-main}"
BOOTSTRAP_MARKER="/root/.homelabv2.4.3-bootstrap-reboot-done"
export HOMELAB_VERSION="2.4.2"
...
safe_reboot_gate() {
  if need_safe_reboot; then
    if [[ ! -f "$BOOTSTRAP_MARKER" ]]; then
      ...
      echo "bash <(curl -fsSL https://raw.githubusercontent.com/bacproxmox/homelabv2.4.3/main/bootstrap.sh)"
      echo
      sleep 8
      reboot
    else
...
echo
echo "Starting Homelab v2.4.3 menu..."
