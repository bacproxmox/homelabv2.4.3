...
add_root_folder_v3(){ local app="$1" url="$2" key="$3" path="$4"; echo; echo "📁 $app root folder kontrolü: $path"; existing="$(api_get "$url" "$key" /rootfolder v3 2>/dev/null || echo '[]')"; echo "$existing" | jq -e --arg path "$path" '.[] | select(.path==$path)' >/dev/null 2>&1 && { echo "✅ $app root folder zaten var."; return 0; }; payload="$(jq -n --arg path "$path" '{path:$path}')"; http="$(api POST "$url" "$key" /rootfolder v3 "$payload" /tmp/${app}-root.out || true)"; if [[ "$http" =~ ^20[012]$ ]]; then echo "✅ $app root folder eklendi."; else echo "⚠️ $app root folder eklenemedi. HTTP=$http"; cat /tmp/${app}-root.out | jq . 2>/dev/null || cat /tmp/${app}-root.out; fi; }
...
post_client(){ local app="$1" url="$2" key="$3" version="$4" payload="$5"; http="$(api POST "$url" "$key" /downloadclient "$version" "$payload" /tmp/${app}-qbit.out || true)"; if [[ "$http" =~ ^20[012]$ ]]; then echo "✅ $app qBittorrent bağlantısı eklendi."; else echo "⚠️ $app qBittorrent eklenemedi. HTTP=$http"; cat /tmp/${app}-qbit.out | jq 'del(.fields[]?.value? | select(type=="string" and length>12))' 2>/dev/null || cat /tmp/${app}-qbit.out; fi; }
