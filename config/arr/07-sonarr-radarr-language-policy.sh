...
ensure_cf(){ local app="$1" url="$2" key="$3" name="$4" pattern="$5"; api GET "$url" "$key" /customformat '' /tmp/cf-list.json >/tmp/http || true; existing="$(jq -r --arg name "$name" '.[]? | select(.name==$name) | .id' /tmp/cf-list.json 2>/dev/null | head -n1)"; if [[ -n "$existing" ]]; then echo "$existing"; return; fi; payload="$(cf_payload "$name" "$pattern")"; http="$(api POST "$url" "$key" /customformat "$payload" /tmp/cf-create.json || true)"; if [[ "$http" =~ ^20[012]$ ]]; then jq -r '.id // empty' /tmp/cf-create.json; else echo "⚠️ $app custom format oluşturulamadı: $name HTTP=$http" >&2; cat /tmp/cf-create.json >&2; echo ""; fi; }
