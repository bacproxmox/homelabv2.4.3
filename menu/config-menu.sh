...
8) Configure Bacscloud local/users + SMTP + hardening + overview cleanup
...
16) Bacscloud Google Login + controlled registration
17) Bacscloud Admin Overview cleanup
18) Run all core config scripts
19) Additionals menu
20) Exit
MENU
  ...
  case "$choice" in
    ...
    16) run config/nextcloud/07-bacscloud-social-login-and-registration.sh ;;
    17) run config/nextcloud/06-bacscloud-admin-overview-cleanup.sh ;;
    18) run config/00-run-all-core-config.sh || true ;;
    19) bash "$ROOT_DIR/menu/additionals-menu.sh" ;;
    20) exit 0 ;;
