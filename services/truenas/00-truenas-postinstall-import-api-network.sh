...
# Homelab v2.4.3 - TrueNAS post-install helper
...
LOG="${LOG_DIR}/truenas-postinstall-v242-$(date +%Y%m%d-%H%M%S).log"

# Guided pipeline already switches VM101 from installer ISO to disk boot before WebUI/SSH validation.
# Keep the old boot-fix behavior for standalone/manual helper use.
for arg in "$@"; do
  case "$arg" in
    --skip-boot-fix) TRUENAS_SKIP_BOOT_FIX=1 ;;
  esac
done
TRUENAS_SKIP_BOOT_FIX="${TRUENAS_SKIP_BOOT_FIX:-0}"
...
say "============================================================"
say " Homelab v2.4.3 - TrueNAS post-install importfix v8"
say "============================================================"
say "Log: $LOG"
say
...
if [[ "$TRUENAS_SKIP_BOOT_FIX" == "1" ]]; then
  ok "TRUENAS_SKIP_BOOT_FIX=1; VM boot-fix adımı atlandı. Guided pipeline bu işi daha önce yaptı."
else
...
REMOTE_LOG="/tmp/homelab-truenas-v242-importfix.log"
